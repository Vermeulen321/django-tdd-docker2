...
def test_add_movie(client: Client):
...
@pytest.mark.django_db
def test_add_movie_invalid_json(client: Client):
    movies = Movie.objects.all()
    assert len(movies) == 0
...
@pytest.mark.django_db
def test_add_movie_invalid_json_keys(client: Client):
    movies = Movie.objects.all()
    assert len(movies) == 0
...
def test_get_single_movie_incorrect_id(client: Client):
